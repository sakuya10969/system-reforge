...
- [x] 1. バックエンド基盤セットアップ
  - [x] 1.1 DB接続・SQLAlchemy AsyncSession設定
    - `server/app/infrastructure/database/connection.py` を作成
  ...
  - [x] 1.2 Projectドメインモデル作成
    - `server/app/domain/models/project.py` にProjectデータクラスを実装
    - `server/app/domain/models/__init__.py` でエクスポート
    - _Requirements: 1.1_
  - [x] 1.3 ProjectRepositoryインターフェース作成
    - `server/app/domain/repositories/project_repository.py` に抽象クラスを実装
    - create, find_by_id, find_all, delete メソッドを定義
    - `server/app/domain/repositories/__init__.py` でエクスポート
    - _Requirements: 1.1, 2.1, 3.1, 4.1_
  - [x] 1.4 ドメイン例外クラス作成
    - `server/app/domain/exceptions.py` にProjectNotFoundErrorを実装
    - _Requirements: 3.2, 4.2_

- [x] 2. Infrastructure層実装
  - [x] 2.1 SQLAlchemyテーブルモデル作成
    - `server/app/infrastructure/database/models.py` にProjectModelを実装
    - UUID主キー、name、description、s3_prefix、created_at、updated_atカラム
    - _Requirements: 1.1_
  - [x] 2.2 SQLAlchemyProjectRepository実装
    - `server/app/infrastructure/database/repositories/project_repository.py` を作成
...
- [x] 4. API層実装
  - [x] 4.1 Pydanticスキーマ作成
    - `server/app/api/schemas/project.py` を作成
  ...
  - [x] 4.4 プロジェクトルーター実装
    ...
    - _Requirements: 1.1, 2.1, 3.1, 4.1_
  - [x] 4.5 FastAPIアプリケーション設定
    - `server/app/main.py` を更新
...
- [x] 5. チェックポイント - バックエンドテスト確認
  - すべてのテストが通ることを確認し、不明点があればユーザーに質問する。

- [x] 6. Alembicマイグレーション設定
  - [x] 6.1 Alembic初期設定
    - `server/app/infrastructure/migrations/` にAlembic設定を作成
    - alembic.ini、env.pyの設定
    - _Requirements: なし（基盤）_
  - [x] 6.2 projectsテーブルマイグレーション作成
